...
def extract_patches(img, patch_size):
    """Extract all overlapping patches of shape (patch_size x patch_size x c)"""
    C, H, W = img.shape
    d = patch_size
    patches = img.unfold(1, d, 1).unfold(2, d, 1)  # shape: (C, H-d+1, W-d+1, d, d)
    patches = patches.permute(1, 2, 0, 3, 4)  # (H-d+1, W-d+1, C, d, d)
    patches = patches.contiguous().view(-1, C * d * d)  # shape: (s, r)
    return patches

def estimate_noise_level_torch(I, d):
    """
    Estimate noise level from image I using patch size d.
    Args:
        I: Input image tensor (C, H, W) in float32
        d: Patch size (int)

    Returns:
        sigma: Estimated noise level (float)
    """
    device = I.device
    r = d * d * I.shape[0]  # c * d^2
    # Step 1: Extract patches
    X = extract_patches(I, d)  # shape: (s, r)
    s = X.shape[0]
    ...
    X_centered = X - mu
    Sigma = (X_centered.T @ X_centered) / s  # shape: (r, r)

    # Step 4: Compute eigenvalues
    eigvals = torch.linalg.eigvalsh(Sigma)
    eigvals = torch.sort(eigvals, descending=True).values  # sorted eigenvalues

    # Step 5-10: Loop to find τ and estimate σ
    for i in range(r):
        subset = eigvals[i:]
        τ = torch.mean(subset)
        if torch.isclose(τ, subset.median(), rtol=1e-5):
            sigma = torch.sqrt(τ).item()
            break
...
### Single Step (Tweedie's Formula) -> Multi Step (DDIM)
def sample_model_fn(x, noise_level, model_diffusion, vec_t=None, model_out_type='pred_xstart', \
        diffusion=None, ddim_sample=False, alphas_cumprod=None, sigma_min = 0.01, num_steps = 2, p = 7, **model_kwargs):
    sqrt_alphas_cumprod     = torch.sqrt(alphas_cumprod)
    sqrt_1m_alphas_cumprod  = torch.sqrt(1. - alphas_cumprod)
    reduced_alpha_cumprod   = torch.div(sqrt_1m_alphas_cumprod, sqrt_alphas_cumprod)        # equivalent noise sigma on image
    sigma_max = noise_level / 255.
    get_time_step_fn = lambda r: (sigma_max ** (1 / p) + r * (sigma_min ** (1 / p) - sigma_max ** (1 / p))) ** p
    time_steps = np.linspace(0, 1, num_steps)
    sigma_steps = [get_time_step_fn(i) for i in time_steps]
    t_steps = [find_nearest(reduced_alpha_cumprod, sigma) for sigma in sigma_steps]
    for i in range(num_steps):
        ...
        if t_step != t_steps[-1]:
            alpha_prod_t = alphas_cumprod[int(t_step)]
            beta_prod_t = 1 - alpha_prod_t
            epsilon = (x - alpha_prod_t ** (0.5) * out) / beta_prod_t ** (0.5)
            alpha_prod_tm1 = alphas_cumprod[int(t_steps[i + 1])]
            beta_prod_tm1 = 1 - alpha_prod_tm1
            x = alpha_prod_tm1 ** (0.5) * out + beta_prod_tm1 ** (0.5) * epsilon
    return out

def ddim_inversion(x, noise_level, model_diffusion, alphas_cumprod=None, diffusion=None, sigma_min = 0.01, num_steps = 5, p = 7, **model_kwargs):
    sqrt_alphas_cumprod     = torch.sqrt(alphas_cumprod)
    sqrt_1m_alphas_cumprod  = torch.sqrt(1. - alphas_cumprod)
    reduced_alpha_cumprod   = torch.div(sqrt_1m_alphas_cumprod, sqrt_alphas_cumprod)        # equivalent noise sigma on image
    ...
    t_steps = [find_nearest(reduced_alpha_cumprod, sigma) for sigma in sigma_steps]
    with torch.no_grad():
        x = x * sqrt_alphas_cumprod[int(t_steps[-1])] + sqrt_1m_alphas_cumprod[int(t_steps[-1])] * torch.randn_like(x)    
